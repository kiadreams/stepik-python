...
async def check_access(participant):
    try:
        if await asyncio.sleep(participant["Уровень секретности"], participant["Срок доступа"]):
            print(f'Участник {participant["Имя"]} {participant["Фамилия"]} имеет действующий доступ. '
                  f'Продолжительность доступа: {participant["Срок доступа"]}')
        else:
            raise ValueError(
                f'Ошибка доступа: У участника {participant["Имя"]} {participant["Фамилия"]} '
                f'срок доступа истек или не указан.')

    except asyncio.CancelledError:
        print(f'Доступ участника {asyncio.current_task().get_name()} отменен из-за критической ошибки.')
...
async def main():
    tasks = [asyncio.create_task(check_access(participant), name=f'{participant["Имя"]} {participant["Фамилия"]}')
             for participant in data]
    res = await asyncio.wait(tasks, return_when=asyncio.FIRST_EXCEPTION)
    # Тут все оставшиеся задачи отменит сам event loop по факту своего завершения.
