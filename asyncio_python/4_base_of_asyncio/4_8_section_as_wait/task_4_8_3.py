...
async def check_access(data_elem: dict[str, str | int | dict[str, str] | list[str]]) -> None:
    try:
        access_period = await asyncio.sleep(data_elem['Уровень секретности'], data_elem.get('Срок доступа'))
    except asyncio.CancelledError:
        print(f'Доступ участника {asyncio.current_task().get_name()} отменен из-за критической ошибки.')
        return
    if access_period is None:
        msg = f'Ошибка доступа: У участника {asyncio.current_task().get_name()} срок доступа истек или не указан.'
        print(msg)
        raise ValueError(msg)
    else:
        print(
            f'Участник {asyncio.current_task().get_name()} имеет действующий доступ. Продолжительность доступа: '
