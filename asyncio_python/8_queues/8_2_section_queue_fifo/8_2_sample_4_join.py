...
async def main():
    ...
    prod_task_1 = asyncio.create_task(
        producer(queue, (0, 10, 2)), name="Производитель_1"
    )
    prod_task_2 = asyncio.create_task(
        producer(queue, (1, 10, 2)), name="Производитель_2"
    )
    cons_task = asyncio.create_task(consumer(queue), name="Потребитель")
    # Список имен незавершенных задач
    # ['Task-1', 'Производитель_1', 'Производитель_2', 'Потребитель']
    ...
    cons_task.cancel()
    # Переключение контекста для выполнения запроса на отмену задачи
    await asyncio.sleep(0)
...
"""
Метод следует использовать с осторожностью, так как в случае, если потребитель будет получать 
элементы из очереди быстрее, чем они туда помещаются, блокировка может быть снята преждевременно, 
до отправки всех элементов в очередь. С другой стороны, если отправка в очередь элементов будет 
производиться Производителями опережающими извлечение темпами, и количество отправляемых элементов 
не будет ограничено, например, элементы будут создаваться в бесконечном цикле, блокировка .join() 
никогда не будет снята, так как очередь никогда не будет "опустошена".
"""