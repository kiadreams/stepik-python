...
async def customer(queue):
    ...
    try:
        ...
        print(
            f"{RED}Попытка получить элемент из пустой очереди методом "
            f"get_nowait() вызвала asyncio.QueueEmpty{END}",
        )
...
async def producer(queue):
    for i in range(5):
        ...
        if queue.full():  # Если очередь заполнена
            ...
            try:
                # Применяем метод put_nowait() для вызова исключения
                # asyncio.QueueFull
                ...
                print(
                    f"{RED}Попытка поместить элемент в заполненную очередь "
