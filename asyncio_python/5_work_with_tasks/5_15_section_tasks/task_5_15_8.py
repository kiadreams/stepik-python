...
async def check_role(msg: dict) -> None:
    role = msg["role"]
    message = msg["message"]
    match role:
        ...
        case "student" | "moderator":
            text = check_message(message)
            if text and role == "student":
                text = "В сообщении есть запрещённое слово, сообщение скрыто"
            message = text if text else message
    print(f"{role}:", message)


async def main() -> None:
    await asyncio.gather(*map(check_role, messages))
