...
async def check_message(msg: str):
    lower_msg = msg.lower()
    for word in banned_words:
        if re.search(rf"(^|\W){word}(\W|$)", lower_msg) is not None:
            asyncio.current_task().cancel()
            await asyncio.sleep(0)
    return f"{asyncio.current_task().get_name()}: {msg}"
...
async def main():
    ...
    [
        print(
            f"В сообщении {task.get_name()} стоп-слово: task.done(): False"
            if task.cancelled()
            else task.result(),
        )
        for task in done
    ...
    # for task in done:
    #     if task.cancelled():
    #         print(f"В сообщении {task.get_name()} стоп-слово: task.done(): False")
    #     else:
    #         print({task.result()})
