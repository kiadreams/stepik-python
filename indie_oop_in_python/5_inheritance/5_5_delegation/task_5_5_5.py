...
class Transport:
    ...
    def __str__(self):
        return (
            f"Тип транспорта {self.kind} марки {self.brand} может развить "
            f"скорость {self.max_speed} км/ч"
        )
...
class Plane(Transport):
    ...
    def __str__(self):
        return (
            f"Самолет марки {self.brand} вмещает в себя {self.capacity} людей"
        )
...
print(
    first_plane
)  # Самолет марки Virgin Atlantic может вмещать в себя 450 людей
...
first_car.gasoline = (
    20  # Печатает 'Объем топлива увеличен на 20 л и составляет 320 л'
)
